...
int main() {
  ...
  Shader::ptr shader(
      new Shader("asserts/camera/camera.vs", "asserts/camera/camera.fs"));
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    float radius = 10.0f;
    float camX = static_cast<float>(std::sin(timeValue) * radius);
    float camZ = static_cast<float>(std::cos(timeValue) * radius);
